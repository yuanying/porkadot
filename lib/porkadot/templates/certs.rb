...
class Porkadot::Templates::Certs
  attr_reader :config
  attr_reader :asset
  attr_reader :logger

  def initialize config, asset
    @config = config
  ...
  def ca_key
    @ca_key ||= private_key(asset.ca_key_path)
    return @ca_key
  end

  def ca_cert(refresh=false)
    return @ca_cert if defined?(@ca_cert)
    if File.file?(asset.ca_cert_path) and !refresh
      self.logger.debug("--> CA cert already exists, skipping: #{asset.ca_cert_path}")
      @ca_cert = OpenSSL::X509::Certificate.new(File.read(asset.ca_cert_path))
    else
      @ca_cert = _ca_cert(asset.ca_cert_path, self.ca_name, self.ca_key)
    end
    return @ca_cert
  end
  ...
  def client_key
    @client_key ||= private_key(asset.client_key_path)
    return @client_key
  end

  def client_cert(refresh=false)
    return @client_cert if defined?(@client_cert)
    if File.file?(asset.client_cert_path) and !refresh
      self.logger.debug("--> Client cert already exists, skipping: #{asset.client_cert_path}")
      @client_cert = OpenSSL::X509::Certificate.new(File.read(asset.client_cert_path))
    else
      @client_cert = _client_cert(asset.client_cert_path, self.client_name, self.client_key, self.ca_cert(false), self.ca_key)
    end
    return @client_cert
  end
...
require 'porkadot/templates/certs/etcd'
require 'porkadot/templates/certs/k8s'
require 'porkadot/templates/certs/front_proxy'
